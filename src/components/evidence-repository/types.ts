...
export type FilterType =
  'all' | 'mandatory' | 'optional' | 'uploaded' | 'pending' | 'approved' | 'rejected' | 'expired';
...
export function calculateFolderMetrics(folder: EvidenceFolder): FolderMetrics {
  ...
  const uploadedDocuments = documents.filter(
    d => d.status !== 'not_uploaded' && d.status !== 'not_applicable'
  ).length;
  ...
  const mandatoryUploaded = documents.filter(
    d => d.mandatory && d.status !== 'not_uploaded' && d.status !== 'not_applicable'
  ).length;
  const completionPercentage =
    requiredDocuments > 0 ? Math.round((mandatoryUploaded / requiredDocuments) * 100) : 100;
...
export function calculateSectionMetrics(
  ...
  visibleFolders.forEach(folder => {
    ...
    totalUploaded += mandatoryDocs.filter(
      d => d.status !== 'not_uploaded' && d.status !== 'not_applicable'
    ).length;
  });
