...
export const TemplatesPage = () => {
  ...
  return (
    ...
    >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/30">
                <th className="text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-4 py-3">Template Name</th>
                <th className="text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-4 py-3 hidden md:table-cell">Category</th>
                <th className="text-center text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-4 py-3">Version</th>
                <th className="text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-4 py-3 hidden lg:table-cell">Uploaded By</th>
                <th className="text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-4 py-3 hidden lg:table-cell">Uploaded Date</th>
                <th className="text-center text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-4 py-3">Status</th>
                <th className="text-center text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-4 py-3 w-[60px]">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    ...
                    <p className="text-xs text-muted-foreground/70 mt-1">Upload a template to get started</p>
                  </td>
                </tr>
              ) : (
                <AnimatePresence>
                  {templates.map((template) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground mt-0.5">
                              {template.fileType.toUpperCase()} • {template.fileSize} • {template.downloads} downloads
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <code className="text-xs bg-muted px-1.5 py-0.5 rounded">v{template.version}</code>
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell">
                        <span className="text-xs text-muted-foreground">{template.uploadedBy}</span>
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell">
                        <span className="text-xs text-muted-foreground">{template.uploadedDate}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
      ...
      <Dialog open={showReplaceDialog} onOpenChange={(open) => { setShowReplaceDialog(open); if (!open) { setReplaceFile(null); setReplaceNotes(''); } }}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="rounded-lg border p-3 bg-muted/20">
              ...
              <p className="text-sm font-medium">v{selectedTemplate?.version} • {selectedTemplate?.fileSize}</p>
            </div>
              ...
              {replaceFile ? (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">{(replaceFile.size / 1024).toFixed(1)} KB</p>
                  </div>
                </div>
              ) : (
                <>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="rounded-lg border p-4 space-y-3 bg-muted/20">
              ...
              <div className="flex items-center justify-between">
                ...
                <code className="text-xs bg-muted px-1.5 py-0.5 rounded">v{selectedTemplate?.version}</code>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="space-y-3">
                {selectedTemplate?.versionHistory.map((version, index) => (
                  <div
                    ...
                    className={cn(
                      'relative pl-6 pb-3',
                      index < (selectedTemplate?.versionHistory.length || 0) - 1 && 'border-l-2 border-muted ml-2'
                    )}
                  >
                    {/* Timeline dot */}
                    <div className={cn(
