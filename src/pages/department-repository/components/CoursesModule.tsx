        ...
        >
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button
              ...
              onClick={handleAddCO}
              disabled={!newCO.description.trim()}
            >
...
const generateMockEvidence = (): EvidenceRecord[] => [
  ...
  {
    id: '2',
    documentName: 'Machine Learning Lab',
    year: 'III Year',
    semester: 'Semester 5',
    uploadedBy: 'Dr. Anita Sharma',
    date: '2025-01-10',
  ...
  {
    id: '5',
    documentName: 'Natural Language Processing',
    year: 'IV Year',
    semester: 'Semester 7',
    uploadedBy: 'Dr. Anita Sharma',
    date: '2025-01-08',
  ...
  },
  {
    ...
    year: 'IV Year',
    semester: 'Semester 7',
    uploadedBy: 'Mr. Anil Reddy',
    date: '2025-01-09',
    status: 'verified',
  },
  {
    id: '7',
    documentName: 'Data Structures',
    year: 'II Year',
    semester: 'Semester 3',
  ...
  {
    id: '8',
    documentName: 'Data Structures Lab',
    year: 'II Year',
    semester: 'Semester 3',
    uploadedBy: 'Dr. Sunita Patel',
    date: '2025-01-05',
...
export const CoursesModule = ({ department, academicYear }: CoursesModuleProps) => {
  ...
  const formCI =
    (parseFloat(newCourse.lectureHours) || 0) + (parseFloat(newCourse.theoryHours) || 0);
  ...
  const filteredEvidence = useMemo(() => {
    let filtered = evidence.filter(e => e.year === selectedYear && e.semester === selectedSemester);
  ...
  const handleDownloadTemplate = useCallback(() => {
    const header =
      'Department,Year,Semester,Course Code,Course Name,Faculty Name,Course Type,Lecture Hours (CI),Theory Hours (CI),Practical Hours (PI),Team Work Hours (TW),Self Learning Hours (SL),Status';
    const sampleRows = [
      ...
      reader.onload = event => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const practicalHrs = parseFloat(
            row['Practical Hours (PI)'] || row['Lab Hours'] || row['Practical Hours'] || '0'
          );
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Manage courses with CI, PI, TW, SL hours — Credits auto-calculated as Total Hours /
                30 (rounded)
              </p>
            </div>
          </div>
              ...
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600 mt-0.5">
                    Total Courses: {totalCoursesForYearSem} • Total Credits:{' '}
                    {semesterStats.totalCredits} • {selectedYear} / {selectedSemester}
                  </p>
                </div>
              </CardContent>
            </Card>
                ...
                <TableBody>
                  {filteredCourses.map((course, idx) => (
                    <TableRow key={course.id} className="hover:bg-muted/20">
                      ...
                      <TableCell className="text-xs text-center font-medium text-blue-600 whitespace-nowrap">
                        {course.ciHours}
                      </TableCell>
                      <TableCell className="text-xs text-center font-medium text-purple-600 whitespace-nowrap">
                        {course.piHours}
                      </TableCell>
                      <TableCell className="text-xs text-center font-medium text-emerald-600 whitespace-nowrap">
                        {course.teamWorkHours}
                      </TableCell>
                      <TableCell className="text-xs text-center font-medium text-amber-600 whitespace-nowrap">
                        {course.selfLearningHours}
                      </TableCell>
                      <TableCell className="text-xs text-center font-bold text-rose-600 whitespace-nowrap">
                        {course.totalHours}
                      </TableCell>
                      <TableCell className="text-center whitespace-nowrap">
                  ...
                  <TableRow className="bg-muted/50 border-t-2 font-semibold">
                    ...
                    <TableCell
                      colSpan={3}
                      className="text-xs text-right font-bold whitespace-nowrap"
                    >
                      SEMESTER TOTALS →
                    </TableCell>
      ...
      {filteredCourses.length > 0 && (
        <Card className="border-border/50">
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground">
              Click on a course name above to view and manage its Course Outcomes and Evidence
              documents.
            </p>
      ...
      <Dialog
        ...
        onOpenChange={open => {
          if (!open) setSelectedCourseForDetail(null);
        }}
      >
          ...
          <DialogHeader>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <Badge variant="outline" className="text-[10px]">
                {selectedCourseForDetail?.courseCode}
              </Badge>
              <Badge variant="outline" className="text-[10px]">
                {selectedCourseForDetail?.courseType}
              </Badge>
              <Badge variant="outline" className="text-[10px]">
                {selectedCourseForDetail?.facultyName}
              </Badge>
              <Badge className="bg-indigo-600 text-white text-[10px]">
                {selectedCourseForDetail?.credits} Credits
              </Badge>
            </div>
          </DialogHeader>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-muted/30">
                        ...
                        <TableHead className="text-[10px] font-semibold w-[80px]">
                          Semester
                        </TableHead>
                        ...
                        <TableHead className="text-[10px] font-semibold text-right w-[120px]">
                          Actions
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                      ...
                      {(() => {
                        const courseEvidence = evidence.filter(
                          e =>
                            e.documentName
                              .toLowerCase()
                              .includes(selectedCourseForDetail.courseName.toLowerCase()) ||
                            e.documentName
                              .toLowerCase()
                              .includes(selectedCourseForDetail.courseCode.toLowerCase())
                        );
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-base">
              {editingCourse ? 'Edit Course' : 'Add Course'} (NBA Format)
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={v => setNewCourse({ ...newCourse, courseType: v })}
                  >
                    <SelectTrigger className="mt-1 h-9 text-sm">
              ...
              <div className="p-3 rounded-lg border bg-muted/30 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-[10px] font-semibold text-emerald-600 mb-1.5">
                      TW — Team Work
                    </p>
                    <Input
                      type="number"
                      min="0"
                      value={newCourse.teamWorkHours}
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      min="0"
                      value={newCourse.selfLearningHours}
                      onChange={e =>
                        setNewCourse({ ...newCourse, selfLearningHours: e.target.value })
                      }
                      placeholder="0"
                      className="h-8 text-sm"
                    />
                ...
                <p className="text-[9px] text-muted-foreground text-center">
                  Credits = (CI + PI + TW + SL) / 30 = {formTotalHours} / 30 ={' '}
                  {(formTotalHours / 30).toFixed(2)} ≈ <strong>{formCredits}</strong>
                </p>
              </div>
                  ...
                  <TableBody>
                    {uploadPreview.map((course, idx) => (
                      <TableRow
                        key={course.id}
                        className={cn(
                          course.validationStatus === 'invalid' &&
                      ...
                      >
                        ...
                        <TableCell className="text-xs text-center text-blue-600">
                          {course.ciHours}
                        </TableCell>
                        <TableCell className="text-xs text-center text-purple-600">
                          {course.piHours}
                        </TableCell>
                        <TableCell className="text-xs text-center text-emerald-600">
                          {course.teamWorkHours}
                        </TableCell>
                        <TableCell className="text-xs text-center text-amber-600">
                          {course.selfLearningHours}
                        </TableCell>
                        <TableCell className="text-xs text-center font-bold text-rose-600">
                          {course.totalHours}
                        </TableCell>
