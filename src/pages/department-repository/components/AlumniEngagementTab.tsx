...
const emptyForm = (): CreateAlumniEngagementRequest => ({
  ...
  activityDate: '',
  role: undefined,
  contributionHours: undefined,
});

type FormData = CreateAlumniEngagementRequest;

export const AlumniEngagementTab = () => {
  const { user } = useAuth();
  ...
  const [records, setRecords] = useState<AlumniEngagementResponse[]>([]);
  ...
  const matchesSearch = (r: AlumniEngagementResponse, q: string): boolean => {
    const lq = q.toLowerCase();
    return (
      r.activityName.toLowerCase().includes(lq) ||
      (r.engagementType?.toLowerCase().includes(lq) ?? false) ||
      (r.role?.toLowerCase().includes(lq) ?? false) ||
      String(r.contributionHours ?? '').includes(lq)
    );
  };
  ...
  const handleCreate = async () => {
    ...
    if (!formData.activityName || !formData.activityDate) {
      toast.error('Activity Name and Activity Date are required');
      return;
    }
    setSaving(true);
    try {
      await alumniService.addEngagement(departmentId, selectedAlumniId, formData);
      toast.success('Engagement record added successfully');
                ...
                <Table className="table-fixed w-full min-w-[800px]">
                  <TableHeader>
                    <TableRow className="bg-muted/40">
                      ...
                      <TableHead className="text-[10px] font-semibold w-28">Activity</TableHead>
