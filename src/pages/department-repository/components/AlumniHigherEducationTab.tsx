...
import { AlumniDetailResponse, AlumniHigherEducationResponse, CreateAlumniHigherEducationRequest } from '@/types/alumni.types';
...
export const AlumniHigherEducationTab = () => {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Alumni ID', 'Institution Name', 'Program Name', 'Country', 'Admission Year', 'Completion Year', 'Status'];
    const sampleRow = ['ALM2020001', 'IIT Bombay', 'M.Tech in AI', 'India', '2020', '2022', 'Completed'];
    const csvContent = `\ufeff${headers.join(',')}\n${sampleRow.join(',')}\n`;
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url; link.download = 'alumni_higher_education_template.csv';
  ...
  const getStatusBadgeVariant = (status: string | null): 'default' | 'secondary' | 'outline' | 'destructive' => {
    switch (status) {
      ...
      case 'Pursuing': return 'secondary';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      ...
      {!selectedAlumniId ? (
        <Card><CardContent className="py-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Choose an alumni to view their higher education records.</p>
        </CardContent></Card>
      ) : (
        <>
          <Card className="border-border/50">
                  ...
                  <TableBody>
                    ...
                    {!loading && !error && filteredRecords.length === 0 && (
                      <TableRow><TableCell colSpan={8} className="text-center py-8">
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                          ...
                          <p className="text-xs">{searchQuery ? 'No records match your search.' : 'No higher education records found.'}</p>
                        </div>
                      </TableCell></TableRow>
                    )}
                    {!loading && !error && filteredRecords.map((rec, index) => (
                      <TableRow key={rec.id} className="hover:bg-muted/20">
                        ...
                        <TableCell className="text-[10px] p-1.5 font-medium">{rec.institutionName}</TableCell>
                        <TableCell className="text-[10px] p-1.5">{rec.programName}</TableCell>
                        <TableCell className="text-[10px] p-1.5">{rec.country || '-'}</TableCell>
                        <TableCell className="text-[10px] p-1.5 font-mono">{rec.admissionYear}</TableCell>
                        <TableCell className="text-[10px] p-1.5 font-mono">{rec.completionYear || '-'}</TableCell>
                        <TableCell className="text-[10px] p-1.5">
                          <Badge variant={getStatusBadgeVariant(rec.status)} className="text-[9px]">
                            {rec.status || '-'}
                          </Badge>
                        </TableCell>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid gap-3 py-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" placeholder="e.g. IIT Bombay" value={formData.institutionName}
                  onChange={(e) => setFormData(p => ({ ...p, institutionName: e.target.value }))} />
              </div>
              <div className="grid gap-1.5">
                <Label className="text-xs font-medium">Program Name *</Label>
                <Input className="h-9 text-xs" placeholder="e.g. M.Tech in AI" value={formData.programName}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" placeholder="e.g. India" value={formData.country ?? ''}
                  onChange={(e) => setFormData(p => ({ ...p, country: e.target.value || undefined }))} />
              </div>
              <div className="grid gap-1.5">
                <Label className="text-xs font-medium">Admission Year *</Label>
                <Input className="h-9 text-xs" placeholder="e.g. 2020" value={formData.admissionYear}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" placeholder="e.g. 2022" value={formData.completionYear ?? ''}
                  onChange={(e) => setFormData(p => ({ ...p, completionYear: e.target.value || undefined }))} />
              </div>
              <div className="grid gap-1.5">
                ...
                <Select value={formData.status || ''}
                  ...
                  <SelectContent>
                    {STATUS_OPTIONS.map((o) => (<SelectItem key={o} value={o} className="text-xs">{o}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button size="sm" className="text-xs" onClick={handleCreate}
              disabled={saving || !formData.institutionName || !formData.programName || !formData.admissionYear}>
