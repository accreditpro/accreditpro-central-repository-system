...
import { RepositoryTabConfig, RepositoryFieldConfig } from '../types';
import {
  evidenceDocuments,
  masterData,
  coordinatorContext,
...
interface RepositoryTabContentProps {
  ...
  repositoryId?: string;
  /** When true, the tab reads/writes the Infrastructure Coordinator backend instead of mock data. */
  liveMode?: boolean;
}

type RowData = Record<string, string>;

/** Convert a backend section record (camelCase field keys) into a display row keyed by csvColumn labels. */
const toDisplayRow = (record: SectionRecord, fields: RepositoryFieldConfig[]): RowData => {
  const row: RowData = {};
  ...
  row.__id = String(record.id);
  row.__workflowStatus = record.workflowStatus || 'draft';
  return row;
};

/** Convert a csvColumn-keyed form row into the camelCase payload the backend expects. */
const toPayload = (formData: RowData, fields: RepositoryFieldConfig[]): Record<string, unknown> => {
  const payload: Record<string, unknown> = {};
  fields.forEach(f => {
    const value = formData[f.csvColumn];
    // Include blanked fields so clearing a value on edit persists server-side.
...
export const RepositoryTabContent = ({ tabConfig, liveMode }: RepositoryTabContentProps) => {
  ...
  // ---- live (backend) state ----
  const [liveRecords, setLiveRecords] = useState<SectionRecord[]>([]);
  const [liveTotal, setLiveTotal] = useState(0);
  const [livePage, setLivePage] = useState(0);
  const [liveTotalPages, setLiveTotalPages] = useState(0);
  const [liveLoading, setLiveLoading] = useState(false);
  const [liveError, setLiveError] = useState<string | null>(null);
  const [liveEvidence, setLiveEvidence] = useState<DocumentRecord[]>([]);
  ...
  const displayRows = useMemo<RowData[]>(
    () => (liveMode ? liveRecords.map(r => toDisplayRow(r, tabConfig.fields)) : tableData),
    [liveMode, liveRecords, tableData, tabConfig.fields]
  );

  const fetchRecords = useCallback(async (search?: string, page?: number) => {
    if (!liveMode) return;
    const targetPage = page ?? livePageRef.current;
    setLiveLoading(true);
    setLiveError(null);
    try {
      const res = await infrastructureRepositoryService.getSectionRecords(tabConfig.id, {
      ...
      setLiveRecords(res.content || []);
      setLiveTotal(res.totalElements || 0);
      setLiveTotalPages(Math.max(res.totalPages || 1, 1));
      setLivePage(targetPage);
      livePageRef.current = targetPage;
    } catch (e) {
      setLiveError(e instanceof Error ? e.message : 'Failed to load records');
    } finally {
      setLiveLoading(false);
    }
  }, [liveMode, tabConfig.id]);
  ...
  }, [liveMode, tabConfig.id, fetchRecords, fetchEvidence]);
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimer.current = window.setTimeout(() => {
      livePageRef.current = 0;
      fetchRecords(value, 0);
    }, 400);
  };

  const handleDownloadTemplate = () => {
    if (liveMode) {
      infrastructureRepositoryService.downloadSectionTemplate(tabConfig.id)
        .catch((e) => setLiveError(e instanceof Error ? e.message : 'Failed to download template'));
      return;
    }
  ...
  const openCreateRow = () => {
    ...
    tabConfig.fields.forEach(f => { empty[f.csvColumn] = ''; });
    setEditFormData(empty);
  ...
  const handleEditRow = (index: number) => {
    setIsCreating(false);
    setEditingRow(index);
    setEditFormData({ ...displayRows[index] });
    setShowEditDialog(true);
  };
  ...
  const handleSaveEdit = async () => {
    if (liveMode) {
      setSaving(true);
      try {
        const payload = toPayload(editFormData, tabConfig.fields);
        if (isCreating || editingRow === null) {
          await infrastructureRepositoryService.createSectionRecord(tabConfig.id, payload);
        } else {
          const id = Number(displayRows[editingRow].__id);
          await infrastructureRepositoryService.updateSectionRecord(tabConfig.id, id, payload);
        }
        setShowEditDialog(false);
        setEditingRow(null);
        fetchRecords(searchQuery || undefined, 0);
      } catch (e) {
        setLiveError(e instanceof Error ? e.message : 'Failed to save record');
      } finally {
        setSaving(false);
      }
      return;
  ...
  const handleDeleteRow = async (index: number) => {
    if (liveMode) {
      const id = Number(displayRows[index].__id);
      try {
        await infrastructureRepositoryService.deleteSectionRecord(tabConfig.id, id);
        fetchRecords(searchQuery || undefined, livePageRef.current);
      } catch (e) {
        setLiveError(e instanceof Error ? e.message : 'Failed to delete record');
      }
      return;
    }
  ...
  const handleCSVUploadComplete = useCallback((data: Record<string, string>[]) => {
    if (liveMode) {
      // Two-phase CSV: the confirm call already persisted the rows. Refresh the list.
    ...
    setShowUploadDialog(false);
  }, [liveMode, fetchRecords, fetchEvidence, searchQuery]);

  const handleDownloadEvidence = (doc: DocumentRecord) => {
    infrastructureRepositoryService.downloadDocument(doc.id, doc.name).catch(() => undefined);
  };

  const handleDeleteEvidence = async (doc: DocumentRecord) => {
    try {
      await infrastructureRepositoryService.deleteDocument(doc.id);
      fetchEvidence();
    } catch (e) {
      setLiveError(e instanceof Error ? e.message : 'Failed to delete document');
    }
  };

  const filteredData = liveMode
    ? displayRows
  ...
  const getFieldInput = (field: RepositoryFieldConfig) => {
    ...
    >
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-[10px]">
              {liveMode ? liveTotal : tableData.length} records
            </Badge>
          </div>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            {!isReadOnly && (
              <Button variant="outline" size="sm" className="text-xs h-8" onClick={openCreateRow}>
      ...
      {tabConfig.fields.length > 0 && (
        <Card className="border-border/50">
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <CardDescription className="text-xs">
                  {liveMode
                    ? `Showing ${filteredData.length} of ${liveTotal} records`
                  ...
                  <TableBody>
                    {filteredData.length === 0 ? (
                      ...
                      filteredData.map((row, index) => (
                        <TableRow key={row.__id || index} className="hover:bg-muted/50">
                          <TableCell className="text-[10px] text-center text-muted-foreground font-mono p-1.5">
                            {liveMode ? livePage * PAGE_SIZE + index + 1 : index + 1}
                          </TableCell>
                          {tabConfig.fields.map(field => (
          ...
          {liveMode && liveTotalPages > 1 && (
            <CardContent className="border-t pt-3">
              <div className="flex items-center justify-between">
                <p className="text-[10px] text-muted-foreground">
                  Page {livePage + 1} of {liveTotalPages} • {liveTotal} records
                </p>
                <div className="flex items-center gap-1">
                  <Button
                    variant="outline" size="icon" className="h-6 w-6"
                    disabled={livePage === 0}
                    onClick={() => fetchRecords(searchQuery || undefined, livePageRef.current - 1)}
                  >
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary" className="text-[10px]">
              {liveMode ? (liveEvidence.length + ' documents') : (tabEvidence.length + ' documents')}
            </Badge>
          </div>
        </CardHeader>
        <CardContent>
            ...
            {liveMode ? (
              evidenceLoading ? (
                ...
                <div className="flex flex-col items-center gap-2 py-8 text-muted-foreground">
                  <FileText className="h-8 w-8 opacity-40" />
                  <p className="text-xs">No supporting documents uploaded for this section.</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {liveEvidence.map((doc) => (
                      <TableRow key={doc.id} className="hover:bg-muted/50">
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <FileText className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                            <span className="text-xs font-medium truncate max-w-[180px]">{doc.title || doc.name}</span>
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-0.5">
                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleDownloadEvidence(doc)}>
                              <DownloadCloud className="h-3 w-3" />
                            </Button>
                            {!isReadOnly && (
                              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleDeleteEvidence(doc)}>
                                <Trash2 className="h-3 w-3 text-red-600" />
                              </Button>
                            )}
                          </div>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-sm">{isCreating ? 'Add Record' : 'Edit Record'}</DialogTitle>
            <DialogDescription className="text-xs">
              {isCreating
                ? 'Enter the record fields below. Fields marked with * are required.'
      ...
      <CSVUploadDialog
        ...
        tabConfig={tabConfig}
        existingData={liveMode ? [] : tableData}
