...
import { AlumniDetailResponse, AlumniAchievementResponse, CreateAlumniAchievementRequest } from '@/types/alumni.types';
...
const CATEGORY_OPTIONS = ['Professional', 'Academic', 'Social', 'Entrepreneurship', 'Research', 'Other'];
...
export const AlumniAchievementTab = () => {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Alumni ID', 'Achievement Title', 'Category', 'Awarding Organization', 'Achievement Date', 'Description'];
    const sampleRow = ['ALM2020001', 'Best Employee Award', 'Professional', 'Google', '2024-03-15', 'Awarded for outstanding performance'];
    const csvContent = `\ufeff${headers.join(',')}\n${sampleRow.join(',')}\n`;
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url; link.download = 'alumni_achievements_template.csv';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      ...
      {!selectedAlumniId ? (
        <Card><CardContent className="py-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Choose an alumni to view their achievement records.</p>
        </CardContent></Card>
      ) : (
        <>
          <Card className="border-border/50">
                  ...
                  <TableBody>
                    ...
                    {!loading && !error && filteredRecords.length === 0 && (
                      <TableRow><TableCell colSpan={7} className="text-center py-8">
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                          ...
                          <p className="text-xs">{searchQuery ? 'No records match your search.' : 'No achievement records found.'}</p>
                        </div>
                      </TableCell></TableRow>
                    )}
                    {!loading && !error && filteredRecords.map((rec, index) => (
                      <TableRow key={rec.id} className="hover:bg-muted/20">
                        ...
                        <TableCell className="text-[10px] p-1.5 font-medium">{rec.achievementTitle}</TableCell>
                        <TableCell className="text-[10px] p-1.5">
                          {rec.achievementCategory ? (
                            <Badge variant="secondary" className="text-[9px]">{rec.achievementCategory}</Badge>
                          ) : '-'}
                        </TableCell>
                        <TableCell className="text-[10px] p-1.5">{rec.awardingOrganization || '-'}</TableCell>
                        <TableCell className="text-[10px] p-1.5 font-mono">{rec.achievementDate}</TableCell>
                        <TableCell className="text-[10px] p-1.5 truncate max-w-[200px]" title={rec.description || ''}>
                          {rec.description || '-'}
                        </TableCell>
          ...
          <div className="grid gap-3 py-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" placeholder="e.g. Best Employee Award" value={formData.achievementTitle}
                  onChange={(e) => setFormData(p => ({ ...p, achievementTitle: e.target.value }))} />
              </div>
              <div className="grid gap-1.5">
                <Label className="text-xs font-medium">Category</Label>
                <Select value={formData.achievementCategory || ''}
                  onValueChange={(v) => setFormData(p => ({ ...p, achievementCategory: v || undefined }))}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" placeholder="e.g. Google" value={formData.awardingOrganization ?? ''}
                  onChange={(e) => setFormData(p => ({ ...p, awardingOrganization: e.target.value || undefined }))} />
              </div>
              <div className="grid gap-1.5">
                <Label className="text-xs font-medium">Achievement Date *</Label>
                <Input className="h-9 text-xs" type="date" value={formData.achievementDate}
