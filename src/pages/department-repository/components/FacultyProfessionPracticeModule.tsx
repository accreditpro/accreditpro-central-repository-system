...
export const FacultyProfessionPracticeModule = ({ department, academicYear }: FacultyProfessionPracticeModuleProps) => {
  ...
  const handleImportUploaded = useCallback(async () => {
    ...
    const headers = ['EMP Code', 'Faculty Name', 'Person Name', 'Designation', 'Organization', 'Course Name', 'Duration', 'Academic Year'];
    const rows = validRecords.map(r => [
      r.empCode, r.facultyName, r.popPersonName, r.popDesignation, 
      r.popOrganization, r.popCourseName, r.popDuration, selectedYear
    ...
    try {
      setIsLoading(true);
      await facultyRepositoryService.uploadProfessionPracticeCSV(departmentId, fileToUpload, selectedYear);
      setSaveSuccess(true); setTimeout(() => setSaveSuccess(false), 3000); await fetchPractices();
    } catch (error) {
  ...
  const handleSaveRecord = useCallback(async () => {
    ...
    try {
      ...
      if (editingRecord) {
        await facultyRepositoryService.updateProfessionPractice(editingRecord.id, selectedYear, departmentId, payload);
        setSaveSuccess(true); setTimeout(() => setSaveSuccess(false), 3000); fetchPractices();
      } else {
        await facultyRepositoryService.createProfessionPractice(selectedYear, departmentId, payload);
        setSaveSuccess(true); setTimeout(() => setSaveSuccess(false), 3000); fetchPractices();
      }
    } catch (error) {
