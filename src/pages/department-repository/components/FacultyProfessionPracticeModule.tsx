...
import {
  getProfessionPractice,
  createProfessionPractice,
  updateProfessionPractice,
  deleteProfessionPractice,
  uploadProfessionPracticeCSV,
...
// ── Normalise API row ────────────────────────────────────────────────────────
function normalise(r: any, year: string): ProfessionPracticeRecord {
  return {
    id: r.id,
    academicYear: r.academicYear ?? year,
    empCode: r.empCode ?? '',
    facultyName: r.facultyName ?? r.name ?? '',
    popPersonName: r.popPersonName ?? r.personName ?? '',
    popDesignation: r.popDesignation ?? r.designation ?? '',
    popOrganization: r.popOrganization ?? r.organization ?? '',
    popCourseName: r.popCourseName ?? r.courseName ?? '',
    popDuration: r.popDuration ?? r.duration ?? '',
  };
}

// ── Component ────────────────────────────────────────────────────────────────
export const FacultyProfessionPracticeModule = ({ department, academicYear, departmentId: propDeptId }: FacultyProfessionPracticeModuleProps) => {
  const { user } = useAuth();
  ...
  const [deleteTarget, setDeleteTarget] = useState<ProfessionPracticeRecord | null>(null);
  ...
  // ── API: Fetch ────────────────────────────────────────────────────────────
  const fetchRecords = useCallback(async (year: string) => {
    if (!departmentId) return;
    setLoading(true);
    try {
      const res = await getProfessionPractice(year, departmentId);
      const items: any[] = res?.data?.content ?? res?.content ?? res?.data ?? res ?? [];
      setRecords(Array.isArray(items) ? items.map((r) => normalise(r, year)) : []);
    } catch (err: any) {
      toast.error(err?.message ?? 'Failed to load Professor of Practice records');
    } finally {
      setLoading(false);
    }
  }, [departmentId]);
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (r) =>
          ...
          r.popOrganization.toLowerCase().includes(q) ||
          r.popPersonName.toLowerCase().includes(q) ||
          r.popCourseName.toLowerCase().includes(q)
      );
    }
    return result;
  }, [records, searchQuery]);
  ...
  const handleDownloadTemplate = useCallback(() => {
    const sampleRows = [
      'EMP001,Dr. Anita Sharma,Mr. Vikram Singh,Senior Engineer,TCS,Cloud Computing,6 months',
      'EMP002,Mr. Rajesh Kumar,Ms. Priya Patel,Tech Lead,Infosys,Machine Learning,1 year',
    ];
    ...
    a.href = url;
    a.download = `faculty_profession_practice_template_${selectedYear}.csv`;
    a.click();
  ...
  const handleFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const record: ProfessionPracticeRecord = {
          id: `upload-${i}`,
  ...
  const handleImportUploaded = useCallback(async () => {
    ...
    try {
      const csvRows = validRows.map((r) =>
        [r.empCode, r.facultyName, r.popPersonName, r.popDesignation, r.popOrganization, r.popCourseName, r.popDuration].join(',')
      );
      const csvContent = [CSV_HEADERS.join(','), ...csvRows].join('\n');
      const blob = new Blob([csvContent], { type: 'text/csv' });
      const file = new File([blob], 'faculty_profession_practice.csv', { type: 'text/csv' });

      await uploadProfessionPracticeCSV(departmentId, file, selectedYear);
      toast.success(`${validRows.length} records uploaded successfully`);
  ...
  }, [uploadPreview, departmentId, selectedYear, fetchRecords]);
  ...
  const handleSaveRecord = useCallback(async () => {
    ...
    try {
      ...
      if (editingRecord?.id) {
        await updateProfessionPractice(editingRecord.id, selectedYear, departmentId, payload);
        toast.success('Record updated successfully');
      } else {
        await createProfessionPractice(selectedYear, departmentId, payload);
  ...
  }, [formData, selectedYear, editingRecord, departmentId, fetchRecords]);

  const handleEdit = useCallback((record: ProfessionPracticeRecord) => {
  ...
  // ── API: Delete ──────────────────────────────────────────────────────────
  const handleDeleteClick = useCallback((record: ProfessionPracticeRecord) => {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    setDeleting(true);
    try {
      await deleteProfessionPractice(deleteTarget.id, selectedYear, departmentId);
  ...
  }, [deleteTarget, departmentId, selectedYear, fetchRecords]);
    ...
    <div className="space-y-6 w-full min-w-0 max-w-full">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-tight">Professor of Practice</h2>
            <p className="text-xs text-muted-foreground">Manage Profession of Practice records — Person, Organization, Course, Duration</p>
          </div>
        </div>
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto">
              <Button variant="ghost" size="sm" onClick={() => fetchRecords(selectedYear)} disabled={loading} className="gap-2">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input placeholder="Search by EMP code, name, organization..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-9 text-sm" />
        </div>
      ...
      <Card className="border-border/50 w-full min-w-0 max-w-full overflow-hidden shadow-sm">
        <CardHeader className="pb-3 border-b border-border/40">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            <FileText className="h-4 w-4 text-amber-600" />Professor of Practice — {selectedYear}
          </CardTitle>
        </CardHeader>
              ...
              <table className="w-full text-xs text-left min-w-[1200px] border-collapse">
                <thead className="sticky top-0 z-20 bg-muted/80 backdrop-blur border-b border-border/60">
                  <tr>
                    ...
                    <th className="p-3 font-semibold whitespace-nowrap w-44">Person Name</th>
                    <th className="p-3 font-semibold whitespace-nowrap w-40">Designation</th>
                    <th className="p-3 font-semibold whitespace-nowrap w-48">Organization</th>
                    <th className="p-3 font-semibold whitespace-nowrap w-48">Course Name</th>
                    <th className="p-3 font-semibold whitespace-nowrap w-32">Duration</th>
                ...
                <tbody className="divide-y divide-border/30">
                  {filteredRecords.map((r, idx) => (
                    <tr key={r.id} className="hover:bg-muted/40 transition-colors">
                      ...
                      <td className="p-3 whitespace-nowrap">{r.popPersonName || '-'}</td>
                      <td className="p-3 whitespace-nowrap font-medium">{r.popDesignation || '-'}</td>
                      <td className="p-3 whitespace-nowrap">{r.popOrganization || '-'}</td>
                      <td className="p-3 whitespace-nowrap">{r.popCourseName || '-'}</td>
                      <td className="p-3 whitespace-nowrap">{r.popDuration || '-'}</td>
      ...
      <Dialog open={showAddDialog} onOpenChange={(open) => { if (!open) { setShowAddDialog(false); setEditingRecord(null); } }}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle className="text-base">{editingRecord ? 'Edit Professor of Practice' : 'Add Professor of Practice'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
            ...
            <div>
              ...
              <Input value={formData.popDuration} onChange={(e) => setFormData({ ...formData, popDuration: e.target.value })} placeholder="e.g., 6 months, 1 year" className="mt-1 h-9 text-sm w-1/2" />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={(open) => { if (!open) { setShowDeleteDialog(false); setDeleteTarget(null); } }}>
        <DialogContent className="sm:max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Are you sure you want to delete the Professor of Practice record for <strong>{deleteTarget?.facultyName}</strong> ({deleteTarget?.empCode})? This action cannot be undone.
          </p>
          <DialogFooter>
                  ...
                  <TableBody>
                    {uploadPreview.map((r, idx) => (
                      <TableRow key={r.id} className={cn(r.validationStatus === 'invalid' && 'bg-red-500/5')}>
                        ...
                        <TableCell className="text-xs">{r.popOrganization}</TableCell>
