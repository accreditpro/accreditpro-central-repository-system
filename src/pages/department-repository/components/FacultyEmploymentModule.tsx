...
const CSV_HEADERS = [
  ...
  'Joining Designation',
  'Total Experience',
  'Experience in Current Institute',
  'Industry Experience',
...
export const FacultyEmploymentModule = ({
  department,
  academicYear,
}: FacultyEmploymentModuleProps) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (filterType && filterType !== 'all') {
      filtered = filtered.filter(r => r.employmentType?.toLowerCase() === filterType.toLowerCase());
    }
    return filtered;
      ...
      reader.onload = event => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const record: EmploymentRecord = {
            ...
            aicteFacultyId: v[2] || '',
            employmentType: v[3] || '',
            natureOfAssociation: v[4] || '',
            dateOfJoiningInstitution: v[5] || '',
            dateOfJoiningDepartment: v[6] || '',
            joiningDesignation: v[7] || '',
            totalExperience: v[8] || '',
            experienceInCurrentInstitute: v[9] || '',
            industryExperience: v[10] || '',
  ...
  const handleImportUploaded = useCallback(async () => {
    ...
    const rows = validRecords.map(r =>
      [
        ...
        r.dateOfJoiningInstitution,
        r.dateOfJoiningDepartment,
        r.joiningDesignation,
        r.totalExperience,
        r.experienceInCurrentInstitute,
        r.industryExperience,
        r.currentDesignationDate,
        selectedYear,
      ]
        .map(val => (val !== null && val !== undefined && val !== '' ? `"${val}"` : ''))
        .join(',')
    );
    ...
    try {
      setIsLoading(true);
      await facultyRepositoryService.uploadFacultyEmploymentCSV(
        departmentId,
        fileToUpload,
        selectedYear
      );
  ...
  const handleSaveRecord = useCallback(async () => {
    ...
    try {
      ...
      if (editingRecord) {
        await facultyRepositoryService.updateFacultyEmployment(
          editingRecord.id,
          selectedYear,
          departmentId,
          payload
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Manage faculty employment details — AICTE ID, Employment Type, Joining Dates,
              Experience
            </p>
          </div>
        </div>
          ...
          >
            <Card className="border-green-500/30 bg-green-500/5">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-green-700">Data Saved Successfully</p>
                  <p className="text-xs text-green-600 mt-0.5">
                    Records: {totalForYear} • Year: {selectedYear}
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        <Select value={filterType} onValueChange={setFilterType}>
          ...
          <SelectContent>
            ...
            {EMPLOYMENT_TYPES.map(t => (
              <SelectItem key={t} value={t.toUpperCase().replace(/[-\s]+/g, '_')}>
                {t}
              </SelectItem>
            ))}
          </SelectContent>
              ...
              <Table className="min-w-[1400px]">
                <TableHeader>
                  <TableRow className="bg-muted/30">
                    <TableHead className="text-xs font-semibold w-8 sticky left-0 bg-background shadow-[1px_0_0_0_rgba(0,0,0,0.1)] z-10">
                      #
                    </TableHead>
                    ...
                    <TableHead className="text-xs font-semibold whitespace-nowrap text-center">
                      Total Exp (Yrs)
                    </TableHead>
                    <TableHead className="text-xs font-semibold whitespace-nowrap text-center">
                      Institute Exp
                    ...
                    <TableHead className="text-xs font-semibold text-right whitespace-nowrap sticky right-0 bg-background shadow-[-1px_0_0_0_rgba(0,0,0,0.1)] z-10">
                      Actions
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredRecords.map((r, idx) => (
                    <TableRow key={r.id} className="hover:bg-muted/20">
                      <TableCell className="text-xs text-muted-foreground sticky left-0 bg-background shadow-[1px_0_0_0_rgba(0,0,0,0.1)] z-10">
                        {idx + 1}
                      </TableCell>
                      ...
                      <TableCell className="text-xs text-center whitespace-nowrap font-medium">
                        {r.totalExperience || '-'}
                      </TableCell>
        ...
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle className="text-base">
              {editingRecord ? 'Edit Employment Info' : 'Add Employment Info'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-xs">AICTE Faculty ID</Label>
                <Input
                  value={formData.aicteFacultyId}
                  onChange={e => setFormData({ ...formData, aicteFacultyId: e.target.value })}
                  placeholder="AICTE12345"
                  className="mt-1 h-9 text-sm"
                />
              </div>
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {EMPLOYMENT_TYPES.map(e => (
                      <SelectItem key={e} value={e.toUpperCase().replace(/[-\s]+/g, '_')}>
                        {e}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  value={formData.natureOfAssociation}
                  onValueChange={v => setFormData({ ...formData, natureOfAssociation: v })}
                >
                  <SelectTrigger className="mt-1 h-9 text-sm">
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
                  <SelectContent>
                    {NATURE_OF_ASSOCIATIONS.map(n => (
                      <SelectItem key={n} value={n.toUpperCase().replace(/[-\s]+/g, '_')}>
                        {n}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={v => setFormData({ ...formData, joiningDesignation: v })}
                >
                  <SelectTrigger className="mt-1 h-9 text-sm">
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-xs">Date of Joining Institution</Label>
                <Input
                  type="date"
                  value={formData.dateOfJoiningInstitution}
                  onChange={e =>
                    setFormData({ ...formData, dateOfJoiningInstitution: e.target.value })
                  }
                  className="mt-1 h-9 text-sm"
                />
              </div>
              <div>
                <Label className="text-xs">Date of Joining Department</Label>
                <Input
                  type="date"
                  value={formData.dateOfJoiningDepartment}
                  onChange={e =>
                    setFormData({ ...formData, dateOfJoiningDepartment: e.target.value })
                  }
                  className="mt-1 h-9 text-sm"
                />
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <Label className="text-xs">Exp. Current Institute</Label>
                <Input
                  value={formData.experienceInCurrentInstitute}
                  onChange={e =>
                    setFormData({ ...formData, experienceInCurrentInstitute: e.target.value })
                  }
                  placeholder="12"
                  className="mt-1 h-9 text-sm"
                />
              ...
              <div>
                <Label className="text-xs">Industry Exp. (Yrs)</Label>
                <Input
            ...
            <div>
              ...
              <Input
                type="date"
                value={formData.currentDesignationDate}
                onChange={e => setFormData({ ...formData, currentDesignationDate: e.target.value })}
                className="mt-1 h-9 text-sm w-1/2"
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!formData.empCode || !formData.facultyName || isLoading}
            >
        ...
        <DialogContent className="sm:max-w-3xl max-h-[80vh]">
                ...
                <Table>
                  ...
                  <TableBody>
                    {uploadPreview.map((r, idx) => (
                      <TableRow
                        key={r.id}
                        className={cn(r.validationStatus === 'invalid' && 'bg-red-500/5')}
                      >
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!uploadStats || uploadStats.valid === 0 || isLoading}
            >
              {isLoading ? 'Importing...' : `Import ${uploadStats?.valid || 0} Valid Records`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
