...
export const FacultyQualificationModule = ({
  department,
  academicYear,
}: FacultyQualificationModuleProps) => {
  ...
  const fetchQualifications = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await facultyRepositoryService.getFacultyQualifications(
        selectedYear,
        departmentId
      );
      if (res) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (filterLevel && filterLevel !== 'all') {
      filtered = filtered.filter(
        r => r.qualificationLevel?.toLowerCase() === filterLevel.toLowerCase()
      );
    }
    return filtered;
  ...
  const handleDownloadTemplate = useCallback(async () => {
    try {
      await facultyRepositoryService.downloadFacultyQualificationsTemplate(
        selectedYear,
        departmentId
      );
    } catch (error) {
  ...
  const handleImportUploaded = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      await facultyRepositoryService.uploadFacultyQualificationsCSV(
        departmentId,
        fileToUpload,
        selectedYear
      );
  ...
  const handleSaveRecord = useCallback(async () => {
    ...
    try {
      ...
      if (editingRecord) {
        await facultyRepositoryService.updateFacultyQualification(
          editingRecord.id,
          selectedYear,
          departmentId,
          payload
        ...
        await facultyRepositoryService.createFacultyQualification(
          selectedYear,
          departmentId,
          payload
        );
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Manage faculty qualifications — Degree, Specialization, University, PhD Status
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input
            placeholder="Search by EMP code, name, degree..."
            value={searchQuery}
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            ...
            Qualification Data — {selectedYear}
          </CardTitle>
        </CardHeader>
                    ...
                    <TableRow key={r.id} className="hover:bg-muted/20">
                      ...
                      <TableCell className="text-center whitespace-nowrap">
                        <Badge
                          ...
                          className={cn(
                            'text-[10px]',
                            r.phdStatus === 'Completed'
                              ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20'
                              : r.phdStatus === 'Pursuing'
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  ...
                  onValueChange={v => setFormData({ ...formData, qualificationLevel: v })}
                >
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={v => setFormData({ ...formData, phdStatus: v })}
                >
                  <SelectTrigger className="mt-1 h-9 text-sm">
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
              ...
              <div>
                <Label className="text-xs">PhD Awarded Date</Label>
                <Input
                  type="date"
                  value={formData.phdAwardedDate}
                  onChange={e => setFormData({ ...formData, phdAwardedDate: e.target.value })}
                  className="mt-1 h-9 text-sm"
                />
              </div>
            </div>
