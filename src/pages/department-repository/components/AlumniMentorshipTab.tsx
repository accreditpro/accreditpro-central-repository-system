...
import { AlumniDetailResponse, AlumniMentorshipResponse, CreateAlumniMentorshipRequest } from '@/types/alumni.types';
...
const MENTORSHIP_TYPE_OPTIONS = ['Career Guidance', 'Technical Mentorship', 'Entrepreneurship Mentorship', 'Research Mentorship', 'Other'];
...
export const AlumniMentorshipTab = () => {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Alumni ID', 'Mentorship Program', 'Mentorship Type', 'Number of Mentees', 'Start Date', 'End Date'];
    const sampleRow = ['ALM2020001', 'Career Guidance Program', 'Career Guidance', '10', '2024-01-01', '2024-06-30'];
    const csvContent = `\ufeff${headers.join(',')}\n${sampleRow.join(',')}\n`;
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url; link.download = 'alumni_mentorship_template.csv';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      ...
      {!selectedAlumniId ? (
        <Card><CardContent className="py-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Choose an alumni to view their mentorship records.</p>
        </CardContent></Card>
      ) : (
        <>
          <Card className="border-border/50">
                  ...
                  <TableBody>
                    ...
                    {!loading && !error && filteredRecords.length === 0 && (
                      <TableRow><TableCell colSpan={7} className="text-center py-8">
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                          ...
                          <p className="text-xs">{searchQuery ? 'No records match your search.' : 'No mentorship records found.'}</p>
                        </div>
                      </TableCell></TableRow>
                    )}
                    {!loading && !error && filteredRecords.map((rec, index) => (
                      <TableRow key={rec.id} className="hover:bg-muted/20">
                        ...
                        <TableCell className="text-[10px] p-1.5 font-medium">{rec.mentorshipProgram}</TableCell>
                        <TableCell className="text-[10px] p-1.5">
                          {rec.mentorshipType ? <Badge variant="outline" className="text-[9px]">{rec.mentorshipType}</Badge> : '-'}
                        </TableCell>
                        <TableCell className="text-[10px] p-1.5 font-mono">{rec.numberOfMentees != null ? rec.numberOfMentees : '-'}</TableCell>
                        <TableCell className="text-[10px] p-1.5 font-mono">{rec.startDate}</TableCell>
                        <TableCell className="text-[10px] p-1.5 font-mono">{rec.endDate || '-'}</TableCell>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid gap-3 py-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" placeholder="e.g. Career Guidance Program" value={formData.mentorshipProgram}
                  onChange={(e) => setFormData(p => ({ ...p, mentorshipProgram: e.target.value }))} />
              </div>
              <div className="grid gap-1.5">
                <Label className="text-xs font-medium">Mentorship Type</Label>
                <Select value={formData.mentorshipType || ''}
                  ...
                  <SelectContent>
                    {MENTORSHIP_TYPE_OPTIONS.map((o) => (<SelectItem key={o} value={o} className="text-xs">{o}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" type="number" min="1" placeholder="e.g. 10"
                  value={formData.numberOfMentees ?? ''}
                  onChange={(e) => setFormData(p => ({ ...p, numberOfMentees: e.target.value ? Number(e.target.value) : undefined }))} />
              </div>
              <div className="grid gap-1.5">
            ...
            <div className="grid grid-cols-1 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input className="h-9 text-xs" type="date" value={formData.endDate ?? ''}
          ...
          <DialogFooter>
            ...
            <Button size="sm" className="text-xs" onClick={handleCreate}
              disabled={saving || !formData.mentorshipProgram || !formData.startDate || (formData.numberOfMentees != null && formData.numberOfMentees < 1)}>
