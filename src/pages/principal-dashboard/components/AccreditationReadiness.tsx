...
import { Trophy, Award, TrendingUp, Loader2, AlertTriangle } from 'lucide-react';
import {
  principalService,
  PrincipalAccreditationDto,
  DeptScoreDto,
} from '@/services/principal.service';
import { NIRF_SHORT } from '../principal-data';
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">
            Department & criterion level readiness
          </p>
        </div>
      </div>
...
function CriterionTable({
  ...
  const criterionAvgs = criteria.map((_, ci) =>
    Math.round(deptRows.reduce((a, d) => a + d.scores[ci], 0) / Math.max(deptRows.length, 1))
  );
  return (
...
const bestDept = (rows: DeptScoreDto[]) =>
  rows.length > 0 ? [...rows].sort((a, b) => b.overall - a.overall)[0] : undefined;

export function AccreditationReadiness() {
  const [data, setData] = useState<PrincipalAccreditationDto | null>(null);
  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center py-24 text-muted-foreground">
        ...
        Unable to load accreditation readiness. Please try again.
      </div>
    );
  }

  const { nba, naac, nirf } = data;
  const nbaCriteria = nba.criteria.map(c => c.name);
  const nbaWeight = nba.criteria.map(c => c.weightage);
  const naacCriteria = naac.criteria.map(c => c.name);
  const naacWeight = naac.criteria.map(c => c.weightage);
  const nirfParameters = nirf.parameters.map(p => p.name);
  const nirfWeight = nirf.parameters.map(p => p.weightage);

  const nbaBest = bestDept(nba.departments);
  const nirfBest =
    nirf.parameters.length > 0
      ? [...nirf.parameters].sort((a, b) => b.score - a.score)[0]
      : undefined;
  const nirfWeakest =
    nirf.parameters.length > 0
      ? [...nirf.parameters].sort((a, b) => a.score - b.score)[0]
      : undefined;
  ...
  return (
    <div className="space-y-6">
      <Tabs defaultValue="nba">
        <TabsList>
          <TabsTrigger value="nba" className="gap-1.5">
            <Trophy className="h-3.5 w-3.5 text-amber-500" /> NBA
          </TabsTrigger>
          <TabsTrigger value="naac" className="gap-1.5">
            <Award className="h-3.5 w-3.5 text-purple-500" /> NAAC
          </TabsTrigger>
          <TabsTrigger value="nirf" className="gap-1.5">
            <TrendingUp className="h-3.5 w-3.5 text-emerald-500" /> NIRF
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="nba" className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard
              ...
              value={nbaBest?.dept ?? '—'}
              sub={nbaBest ? `${nbaBest.overall}% readiness` : undefined}
            ...
            <StatCard
              ...
              value={`${nba.departments.filter(d => d.overall < 70).length}`}
              sub="Below 70%"
              tone="text-red-600"
              iconBg="bg-red-50 dark:bg-red-950/40"
            />
          </div>
          <CriterionTable deptRows={nba.departments} criteria={nbaCriteria} weight={nbaWeight} />
        </TabsContent>
        ...
        <TabsContent value="naac" className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard
              icon={Award}
              label="Departments at Risk"
              value={`${naac.departments.filter(d => d.overall < 70).length}`}
              sub="Below 70%"
              tone="text-red-600"
              iconBg="bg-red-50 dark:bg-red-950/40"
            />
          ...
          {/* Institution criterion-wise (from API) */}
          <Card>
            <CardHeader className="pb-2">
          ...
          <CriterionTable deptRows={naac.departments} criteria={naacCriteria} weight={naacWeight} />
        </TabsContent>
        ...
        <TabsContent value="nirf" className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard
              ...
              label="Best Category"
              value={nirfBest?.name ?? '—'}
              sub={nirfBest ? `${nirfBest.score}/100` : undefined}
              tone="text-emerald-600"
              iconBg="bg-emerald-50 dark:bg-emerald-950/40"
            />
            <StatCard
              ...
              value={nirfWeakest?.name ?? '—'}
              sub={nirfWeakest ? `${nirfWeakest.score}/100` : undefined}
              tone="text-red-600"
              iconBg="bg-red-50 dark:bg-red-950/40"
            />
          </div>
          ...
          <CriterionTable
            deptRows={nirf.departments}
            criteria={nirfParameters}
            weight={nirfWeight}
            headers={NIRF_SHORT}
          />
        </TabsContent>
