...
const statusColor = (score: number) =>
  score >= 85 ? 'bg-emerald-500/15 text-emerald-600' : score >= 70 ? 'bg-amber-500/15 text-amber-600' : 'bg-red-500/15 text-red-600';
...
export function DepartmentPerformance() {
  ...
  const filtered = useMemo(
    () =>
      departmentRepositories.filter((d) => {
        const matchesDept = dept === 'all' || d.code === dept;
  ...
  const avgReadiness = Math.round(filtered.reduce((a, d) => a + d.readiness, 0) / Math.max(filtered.length, 1));
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-3">
          <FilterBar>
            ...
            <FilterSelect value={program} onValueChange={setProgram} options={programOptions} placeholder="Program" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={Building} label="Departments" value={`${filtered.length}`} sub={`of ${departmentRepositories.length}`} tone="text-indigo-600" iconBg="bg-indigo-50 dark:bg-indigo-950/40" />
        <StatCard icon={BarChart3} label="Avg Repository Readiness" value={`${avgReadiness}%`} sub="Across selected" tone="text-blue-600" iconBg="bg-blue-50 dark:bg-blue-950/40" />
        {best && <StatCard icon={TrendingUp} label="Top Department" value={best.code} sub={`${best.readiness}% readiness`} tone="text-emerald-600" iconBg="bg-emerald-50 dark:bg-emerald-950/40" />}
        {worst && <StatCard icon={TrendingDown} label="Needs Attention" value={worst.code} sub={`${worst.readiness}% readiness`} tone="text-red-600" iconBg="bg-red-50 dark:bg-red-950/40" />}
      </div>
              ...
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  {departmentRepositories.map((d) => (
                    <th key={d.code} className="text-center p-2.5 font-medium">{d.code}</th>
                  ))}
                  <th className="text-center p-2.5 font-medium text-muted-foreground">Institution</th>
                </tr>
              </thead>
              <tbody>
                {departmentRepositories[0].repositories.map((repoRow, ri) => {
                  const instAvg = Math.round(
                    departmentRepositories.reduce((a, d) => a + d.repositories[ri].completion, 0) / departmentRepositories.length
                  );
                  return (
                      ...
                      {departmentRepositories.map((d) => {
                        ...
                        return (
                          <td key={d.code} className="p-2.5 text-center">
                            <span className={`inline-block min-w-[52px] rounded-md px-1.5 py-0.5 font-semibold ${statusColor(cell.completion)}`}>
                              {cell.completion}%
                            </span>
                          </td>
                        );
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-left p-2.5 font-medium text-muted-foreground">Repository</th>
                  <th className="text-left p-2.5 font-medium text-muted-foreground w-40">Completion</th>
