...
import { principalService, DeptInfraDto, InfraAlertDto } from '@/services/principal.service';
...
export function InfrastructureReadiness() {
  ...
  const [data, setData] = useState<{ departments: DeptInfraDto[]; alerts: InfraAlertDto[] } | null>(
    null
  );
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-24 text-muted-foreground">
        ...
        Loading infrastructure readiness...
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center py-24 text-muted-foreground">
        ...
        Unable to load infrastructure data. Please try again.
      </div>
    );
  }

  const deptInfra = data.departments ?? [];
  const infraAlerts = data.alerts ?? [];
  const rows = deptInfra.filter(d => d.dept.toLowerCase().includes(search.toLowerCase()));

  const avg = (fn: (d: DeptInfraDto) => number) =>
    Math.round(deptInfra.reduce((a, d) => a + fn(d), 0) / Math.max(deptInfra.length, 1));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <StatCard
          ...
          value={`${avg(d => d.softwareLicenses)}%`}
          tone="text-amber-600"
        ...
        <StatCard
          ...
          value={`${avg(d => d.evidenceCompletion)}%`}
        ...
        <Card className="border-amber-500/30">
          ...
          <CardContent className="space-y-2">
            {infraAlerts.map((a, i) => (
              <div
                key={i}
                className="flex items-start gap-2.5 rounded-lg border border-amber-500/20 bg-amber-500/5 p-2.5"
              >
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold">
            Department-wise Infrastructure Readiness — {year}
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-center p-2.5 font-medium text-violet-600">
                    Smart Classrooms
                  </th>
                ...
                {rows.map(d => (
                  <tr key={d.dept} className="border-b last:border-0 hover:bg-muted/40">
                    ...
                    <td className="p-2.5 text-center">
                      <ScoreCell value={d.laboratories} />
                    </td>
                    <td className="p-2.5 text-center">
                      <ScoreCell value={d.equipment} />
                    </td>
                    <td className="p-2.5 text-center">
                      <ScoreCell value={d.softwareLicenses} />
                    </td>
                    <td className="p-2.5 text-center">
                      <ScoreCell value={d.ictFacilities} />
                    </td>
                    <td className="p-2.5 text-center">
                      <ScoreCell value={d.smartClassrooms} />
                    </td>
                    <td className="p-2.5">
                    ...
                    <td className="p-2.5 text-center">
                      {d.alerts.length > 0 ? (
                        ...
                        <Badge
                          variant="outline"
                          className="text-[9px] text-emerald-600 border-emerald-500/30"
                        >
                          Clear
                        </Badge>
                      )}
