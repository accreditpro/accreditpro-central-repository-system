...
import { principalService, DeptStudentDto } from '@/services/principal.service';
...
export function StudentPerformance() {
  ...
  const [data, setData] = useState<{ departments: DeptStudentDto[] } | null>(null);
  ...
  const deptStudent = data.departments ?? [];
  ...
  const avg = (fn: (d: DeptStudentDto) => number) =>
    Math.round(deptStudent.reduce((a, d) => a + fn(d), 0) / Math.max(deptStudent.length, 1));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
        ...
        <StatCard
          icon={Briefcase}
          label="Internships"
          value={`${deptStudent.reduce((a, d) => a + d.internships, 0)}`}
          tone="text-cyan-600"
        ...
        <StatCard
          icon={FolderKanban}
          label="Projects"
          value={`${deptStudent.reduce((a, d) => a + d.projects, 0)}`}
        ...
        <StatCard
          icon={Trophy}
          label="Awards"
          value={`${deptStudent.reduce((a, d) => a + d.awards, 0)}`}
          tone="text-amber-600"
        ...
        <StatCard
          ...
          value={`${deptStudent.reduce((a, d) => a + d.certifications, 0)}`}
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold">
            Department-wise Student Performance — {year}
          </CardTitle>
        </CardHeader>
        <CardContent>
