...
import { principalService, PrincipalDashboardDto } from '@/services/principal.service';
    ...
    <div className="relative w-36 h-36 mx-auto">
      <svg className="w-36 h-36 -rotate-90" viewBox="0 0 120 120">
        ...
        <circle
          ...
          strokeLinecap="round"
          className={
            value >= 85 ? 'text-emerald-500' : value >= 70 ? 'text-amber-500' : 'text-red-500'
          }
...
export function ExecutiveDashboard() {
  ...
  const [dashboard, setDashboard] = useState<PrincipalDashboardDto | null>(null);
  ...
  if (!dashboard) {
    return (
      <div className="flex items-center justify-center py-24 text-muted-foreground">
        ...
        Unable to load executive dashboard. Please try again.
      </div>
    );
  }

  const { kpi, institutionStats, departments, criticalGaps: criticalGapsList } = dashboard;
  const kpiData = kpi;

  const kpiCards = [
    {
      label: 'Overall Repository Readiness',
    ...
    },
    {
      ...
      value: `${institutionStats.departments}`,
      icon: Building,
    ...
    },
    {
      ...
      value: `${institutionStats.programs}`,
      icon: BookOpen,
      tone: 'text-cyan-600',
    ...
    {
      label: 'Total Faculty',
      value: `${institutionStats.faculty}`,
      icon: Users,
    ...
    },
    {
      ...
      value: `${institutionStats.students.toLocaleString()}`,
      icon: Users,
      tone: 'text-teal-600',
      bg: 'bg-teal-50 dark:bg-teal-950/40',
    },
    {
      label: 'Overall Evidence Completion',
    ...
    {
      label: 'IQAC Observations',
      value: `${kpiData.iqacObservations}`,
      icon: ClipboardList,
      tone: 'text-sky-600',
    ...
    {
      label: 'Critical Gaps',
      value: `${criticalGapsList.filter(g => g.priority === 'critical').length}`,
      icon: AlertTriangle,
      tone: 'text-red-600',
  ...
  const criticalGaps = criticalGapsList.filter(g => g.priority === 'critical').length;
  // Display derivation from the real evidence-completion KPI (the backend
  // serves the overall completion; the approved/missing split is a read-only
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                onClick={() => navigate('/app/principal-dashboard?view=departments')}
              >
          ...
          <CardContent>
                ...
                <tbody>
                  {(departments ?? []).map(dept => (
                    ...
                    >
                      ...
                      <td className="p-2.5 text-center">{dept.nba}%</td>
                      <td className="p-2.5 text-center">{dept.naac}%</td>
                      <td className="p-2.5 text-center">{dept.nirf}%</td>
                      <td className="p-2.5 text-center">
            ...
            <div className="flex items-center gap-4 mt-3 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-emerald-500" /> Ready (≥85%)
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-amber-500" /> Needs Attention (70–84%)
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-red-500" /> Critical (&lt;70%)
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex items-center gap-3 rounded-xl border border-red-500/30 bg-red-500/5 p-3">
          ...
          <p className="text-xs text-red-700 dark:text-red-400">
            ...
            {criticalGapsList
              ...
              .map(g => `${g.department} (${g.repository})`)
              .join(', ')}
            .
          </p>
